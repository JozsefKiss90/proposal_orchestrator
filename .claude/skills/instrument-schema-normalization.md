...
- **Critical — `instrument_type` value**: The `instrument_type` field in each registry entry MUST be set to the `resolved_instrument_type` value received from the invoking agent context (sourced from `selected_call.json`). Do NOT derive the instrument_type from the application form filename. The artifact schema specification (§8, `section_schema_registry.instrument_type`) explicitly requires this field to match `selected_call.json.instrument_type` exactly. Using a filename-derived identifier (e.g., `"HE-RIA-IA"` from `af_he-ria-ia_en.pdf`) instead of the canonical short form (e.g., `"RIA"`) causes the Phase 1 exit gate predicate `instrument_type_matches_schema` to fail.
