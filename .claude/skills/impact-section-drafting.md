...
## Impact-Specific Canonicalization Rules (GATE-CRITICAL)

**1. Impact Pathways Must Not Compress Metrics:**
- Any objective referenced in B.2.1 or B.2.2 MUST preserve ALL `measurable_target` components from `architecture_inputs/objectives.json`.
- `claim_statuses` MUST include complete metric text for each referenced objective, not only shorthand.
- On violation: return INCOMPLETE_OUTPUT with `failure_reason="Impact section compresses metrics for objective '<obj_id>': missing '<missing_metric>'"`.

**2. Impact Pathways Must Not Assign Guessed WP Sets:**
- If `impact_architecture.json` does not explicitly link a pathway/objective/output to a WP set, avoid exact WP attribution.
- If a WP set is used, `source_ref` MUST point to the artifact field that contains that WP set.
- Do not combine WPs from multiple artifacts unless they are explicitly consistent.
- On violation: return INCOMPLETE_OUTPUT with `failure_reason="Impact section assigns unsourced WP set for pathway/objective '<id>'"`.

**3. Component Terminology in Impact:**
- Impact MUST use canonical objective/outcome component names from `objectives.json` and `outcomes.json`.
- Impact MUST NOT use WP labels as substitutes for canonical system/component titles.
- When the context discusses an objective, use the objective's canonical title. When the context discusses an outcome, use the outcome's canonical title.
- On violation: return INCOMPLETE_OUTPUT with `failure_reason="Impact section uses WP label '<wp_label>' as substitute for canonical component title '<canonical_title>'"`.
