...
Use objective IDs, titles, outcome names, impact pathway names, and KPI labels exactly as stated in Tier 3/Tier 4 artifacts. Preserve all quantified metrics (values, comparators, units) for any objective or impact target explicitly used in the Impact prose — do not alter numeric values, drop conjunctive metrics, or substitute units. Use canonical component/system names from `objectives.json` and `outcomes.json` — do not intentionally rename them (e.g., do not write "framework" when the source says "engine"). Do not describe KPIs as deliverables; cite deliverable IDs with their canonical title and due month from `wp_structure.json`. Do not invent WP mappings; cite explicit mappings from `impact_architecture.json` or `wp_structure.json`, or omit WP lists. Use partner `short_name` or full `legal_name` — do not truncate legal names by dropping suffixes.

These conventions are enforced deterministically by gate predicates (gate_10b, gate_10d). The drafting skill's job is to produce evaluator-oriented content following these conventions — not to perform exhaustive post-hoc validation.
