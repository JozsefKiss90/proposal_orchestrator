...
- Step 2.5: Build the `compliance_checklist` array. Source: the eligibility and compliance section of the evaluation form (if present), and the eligibility_conditions.json from Tier 2B (if already written by call-requirements-extraction). For each compliance requirement identified: assign `requirement_id` (unique, e.g., "CR-01"), `description`, `status` (one of: confirmed, requires_review, not_applicable), `source_section`, `source_document`. The compliance_checklist must not be empty; if no compliance requirements are found in the evaluation form, generate at least one entry from the instrument's known minimum consortium requirements with status: requires_review.
