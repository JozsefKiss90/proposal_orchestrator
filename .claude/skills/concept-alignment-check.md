...
Return your output as a single JSON object in your response. The response must include concept_refinement_summary fields flat at the top level (schema_id, run_id, topic_mapping_rationale, scope_coverage, scope_conflict_log, strategic_differentiation). Do NOT include topic_mapping or compliance_profile in your response — those are produced by a separate downstream skill. Returning an empty object `{}` is invalid and will be treated as a failure.
...
|------|----------|-----------|----------------------------------------------------------|-----------------|-------------------|
| `docs/tier4_orchestration_state/phase_outputs/phase2_concept_refinement/concept_refinement_summary.json` | concept_refinement_summary.json | `orch.phase2.concept_refinement_summary.v1` | schema_id, run_id, topic_mapping_rationale (object: topic_element_id, mapping_to_concept, tier2b_source_ref, tier3_evidence_ref per entry), scope_coverage (object: keyed by scope_element_id, each entry: scope_element_id, constraint_ref, coverage_status, coverage_description, tier2b_source_ref, tier3_evidence_ref, notes), scope_conflict_log (array: conflict_id, description, resolution_status, resolution_note, tier2b_source_ref per entry), strategic_differentiation (string, non-empty) | Yes | topic_mapping_rationale entries derived from expected_outcomes.json entries matched against project_brief content; scope_coverage derived from evaluating all mandatory scope requirements and call constraints against project_brief content; scope_conflict_log derived from scope mismatches found; strategic_differentiation derived from strategic_positioning document in project_brief |
...
**Note:** `artifact_status` must be ABSENT at write time for concept_refinement_summary.json; the runner stamps it post-gate. `topic_mapping.json` and `compliance_profile.json` are produced by the downstream `concept-call-binding-derivation` skill, not by this skill.
...
- Step 2.7a: **Build scope_coverage** — Iterate ALL mandatory Tier 2B scope requirements (from `scope_requirements.json`, field `mandatory: true`) and ALL call constraints (from `call_constraints.json`). For each element:
  - Search the concept corpus for explicit evidence of coverage (named mechanism, stated commitment, described activity).
  - Assign `coverage_status` per the following enum:
    - **"covered"** — explicit Tier 3 evidence exists in the concept corpus. `tier3_evidence_ref` identifies the specific file and location.
    - **"partially_covered"** — the concept addresses the requirement but incompletely (e.g., mentions the topic but lacks a specific mechanism). Not blocking for gate passage.
    - **"unresolved"** — the concept does not address this requirement, OR the coverage is contradictory. MUST also appear in `scope_conflict_log`.
    - **"not_applicable"** — the requirement does not apply to this proposal. MUST include justification in `notes` field, grounded in Tier 2B source.
  - Populate: `scope_element_id` (SR-xx or CC-xx), `constraint_ref` (cross-reference to related CC-xx or SR-xx if applicable, else null), `coverage_status`, `coverage_description` (1–3 sentences explaining how the concept addresses this element), `tier2b_source_ref` (source_section + source_document from the requirement entry), `tier3_evidence_ref` (file + location in project_brief/ where coverage evidence appears), `notes` (optional).
  - **Fail-closed rule**: Any mandatory scope requirement NOT present in `scope_coverage` is treated as unresolved by the downstream gate predicate.
  - **Consistency rule**: If `coverage_status == "unresolved"`, the element MUST also appear in `scope_conflict_log`. If `scope_conflict_log` is empty, ALL mandatory elements MUST have `coverage_status` of "covered", "partially_covered", or "not_applicable".
  - Do NOT auto-mark elements as "covered" without identifying specific Tier 3 evidence. Do NOT suppress legitimate gaps. This is a semantic contract completion, not a cosmetic fix.
- Step 2.8: Build the `scope_conflict_log` array: all framing mismatches (from Step 2.6), scope conflicts (from Step 2.7), and any elements from scope_coverage with `coverage_status == "unresolved"` (from Step 2.7a). If none found, the array is empty. Each entry must have: `conflict_id`, `description`, `resolution_status` (resolved / unresolved), `resolution_note` (required when resolved), `tier2b_source_ref`, `conflict_type` (framing_mismatch / scope_violation / conditional_unmet / coverage_gap).
...
- Step 2.10: Build the decision log entry: compile `alignment_findings` (summary of Confirmed/Inferred/Assumed/Unresolved counts), `uncovered_outcomes` (list of outcome_ids with status Assumed or Unresolved), `vocabulary_gaps` (aggregate list of terms from all outcomes), `tier2b_source_refs` (list of source_section + source_document pairs consulted), `scope_coverage_summary` (counts by coverage_status).
*Note: topic_mapping and compliance_profile derivation (formerly Steps 2.11–2.12) has been moved to the separate `concept-call-binding-derivation` skill, which runs after this skill and reads `topic_mapping_rationale` from the concept_refinement_summary.json produced here.*
...
- `scope_coverage`: derived from Step 2.7a — object keyed by scope_element_id (SR-xx or CC-xx); each value: `{scope_element_id, constraint_ref, coverage_status, coverage_description, tier2b_source_ref, tier3_evidence_ref, notes}`
...
*Note: `topic_mapping` and `compliance_profile` output construction has been moved to the `concept-call-binding-derivation` skill.*
...
*Note: `topic_mapping.json` and `compliance_profile.json` (formerly Steps 5.3–5.4) are written by the downstream `concept-call-binding-derivation` skill.*
...
| scope_coverage | true | ✓ Implemented | Built in Step 2.7a with scope_element_id, constraint_ref, coverage_status (enum: covered/partially_covered/unresolved/not_applicable), coverage_description, tier2b_source_ref, tier3_evidence_ref per entry. Must include all mandatory scope requirements and relevant call constraints. |
...
*Note: Schema validation for `topic_mapping.json` and `compliance_profile.json` is performed by the `concept-call-binding-derivation` skill which now owns those outputs.*
